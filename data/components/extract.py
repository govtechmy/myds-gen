...
def extract_comp_dict(filename):
    with open(f"data/myds/apps/docs/content/docs/develop/(components)/{filename}.mdx") as f:
        data = f.readlines()
    full_text = "".join(data)
    ...
    prop_dict ={i.split("\n")[0].strip(): re.findall(r"<TypeTable\s*?type={{([\S\s]+?)}}\s*?\/>", i) for i in props.split("###")[1:]}
    prop_dict = {i: eval("{"+re.sub(r'\s\s\w+:',prop_proc,y).replace('"`','"').replace('`"','"')+"}") for i,x in prop_dict.items() for y in x}
    return {
        "name": name,
        "description": description,
        "docs":{
            "import": usage_doc[0],
            "use": usage_doc[1].replace("<AlertDialog>", '<AlertDialog variant="default">'), # force usage of variant param on alert dialog
            "props": prop_dict,
...
def generate_json():
    myds_docs_json = []
    comp_files = [i for i in os.listdir("data/myds/apps/docs/content/docs/develop/(components)") if ".ms.mdx" not in i]
