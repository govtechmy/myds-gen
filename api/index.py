...
async def assemble_context(
    task: BaseContext, api_key: str = Security(get_api_key)
) -> str:
...
async def generate_component(
    task: ToGenerate, api_key: str = Security(get_api_key)
) -> str:
...
async def validate_code(
    component_str: ComponentCode, api_key: str = Security(get_api_key)
) -> str:
    try:
...
async def task_planning_iterate(
    task: ToPlanIterate, api_key: str = Security(get_api_key)
) -> BaseTaskWthUpdate:
...
async def wireframe_iterate(
    task: ContextIterate, api_key: str = Security(get_api_key)
) -> str:
...
async def assemble_context_iterate(
    task: ContextIterate, api_key: str = Security(get_api_key)
) -> str:
...
async def generate_component_iterate(
    task: GenIterate, api_key: str = Security(get_api_key)
) -> str:
